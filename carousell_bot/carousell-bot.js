...
bot.onText(/\/item/, (msg) => {
  ...
  msg.prevListings = [];
  chats.push(msg);
  item = msg.text.substr(6);
  bot.sendMessage(
    chatId,
    "You will start receiving " + item + " updates on " + msg.chat.title
  );
});
      ...
      (async () => {
        ...
        await page.goto(
          "https://sg.carousell.com/search/" +
            encodeURIComponent(chat.text.substr(6)) +
        ...
        if (chat.prevListings.length == 0)
          console.log("Script starting... we populate the listings!");
        else {
          diffListings = compareListings(chat.prevListings, listings);
          if (diffListings.length == 0)
            ...
            telegram_bot_sendtext(chat, messages)
          }
        }
        console.log(chat.prevListings)
        //  Save for comparison later
        chat.prevListings = listings;
...
function telegram_bot_sendtext(chat, bot_message_array) {
  bot_message_array.forEach((bot_message) => {
    bot.sendMessage(chat.chat.id, bot_message);
  });
}
